...
#import "IDBPagingView.h"

@interface IDBPage (Private)

@property(nonatomic, assign) NSUInteger index;

@end

@implementation IDBPagingView
...
- (IDBPage *)pageAtLocation:(CGPoint)location {
    ...
    for (IDBPage *page in self.visiblePages) {
        if (page.index == index) {
            return page;
        }
    }
...
- (void)reloadPages {
    for (IDBPage *page in _visiblePages) {
        page.hidden = YES;
...
- (UIView *)dequeueReusablePage {
    IDBPage *page = [_recycledPages anyObject];
    if (page != nil) {
...
- (void)tilePages:(CGPoint)offset {
    ...
    for (NSNumber *number in indexes) {
        ...
        IDBPage *page = [_pagingDataSource pagingView:self pageForIndex:i % _numberOfPages];
        page.index = i;
